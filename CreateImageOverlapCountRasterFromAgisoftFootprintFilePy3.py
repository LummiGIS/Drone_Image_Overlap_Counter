...
try:
    ...
    input_FC = r"Z:\GISpublic\GerryG\Python3\Agisoft_Tools\Agisoft_Image_Overlap_Counter\Test\testdata2.shp"
    
    #this is the path to an empty directory.  This directory will store the interum data.... 
    outFolder = r"Z:\GISpublic\GerryG\Python3\Agisoft_Tools\Agisoft_Image_Overlap_Counter\Test\temp"
    
    #This is the full path and name of the output raster that will store the image overlap count....
    outRas = r"Z:\GISpublic\GerryG\Python3\Agisoft_Tools\Agisoft_Image_Overlap_Counter\Test\temp\imagecount"
    ...
    with arcpy.da.SearchCursor(layer,OID_field) as cursor: # create cursor  
        for row in cursor: 
            ...
            temp_out_file = outFolder+"\\temp"+str(row[0])
            print(temp_out_file)
            arcpy.FeatureToRaster_conversion (layer, fields[1], temp_out_file, cellSize)
